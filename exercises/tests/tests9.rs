// tests9.rs
//
// Rust is highly capable of sharing FFI（外部函数接口：允许一种编程语言调用另一种语言编写的函数或使用其库）
// interfaces with C/C++ and other statically compiled languages, and it can even link within
// the code itself! It makes it through the extern block, just like the code below.
//
// ABI：应用二进制接口
...
//
// Rust mangles（混淆） symbols by default, just like C++ does. To suppress this behavior and make
...
// 在 Rust 中，编译器默认会对函数名进行名称混淆（也称为“符号重整”），这是为了确保不同模块中的函数名不会冲突
extern "Rust" {
...
mod Foo {
    ...
    #[no_mangle]
    // 告诉编译器不要修改这个函数的名称，使得它可以在外部以原始名字引用。允许你为函数指定一个不同的符号名称。
    fn my_demo_function(a: u32) -> u32 {
        a
    }
}
